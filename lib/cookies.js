  ...
  function redirectCookiesWith(data) {
    ...
    if (uri.query[REDIRECT_QUERY_PARAM]) {
      ...
      var setCookieHeaders = Object.keys(cookies).map(function (name) {
        ...
        const cloakedPath = config.prefix + crypto.createHash("sha256").update(nextUri.protocol + "//" + nextUri.host).digest("hex").slice(0, 8) + "/";
  ...
  function rewriteCookiesAndLinks(data) {
    ...
    if (cookies.length) {
      ...
      data.headers["set-cookie"] = cookies.map(function (cookie) {
        ...
        const cloakedPath = config.prefix + crypto.createHash("sha256").update(targetUri.protocol + "//" + targetUri.host).digest("hex").slice(0, 8) + (cookie.path || "/");
        cookie.path = cloakedPath;
    ...
    function updateLink(proxiedUrl, url /*, subdomain*/) {
      ...
      if (next_uri.protocol != uri.protocol || next_uri.host != uri.host) {
        ...
        const cloakedPath = config.prefix + crypto.createHash("sha256").update(uri.protocol + "//" + uri.host).digest("hex").slice(0, 8);
